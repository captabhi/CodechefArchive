...
int main()
{
	...
	while(t--)
	{
		long long int n,i;
		cin>>n;
		long long int initialH[n],targetH[n],validH,sum=0,j,count=n,maxRL,maxLL,countopr=0,flag=0,cntplant=0,maxcntplant=0;
		int bitarr[n];
		for(i=0;i<n;i++)
		bitarr[i]=1;
		for(i=0;i<n;i++)
		...
		while(count>0)
	{
		...
		
		{
			...
			//out<<"Current Range CameOut to be from "<<i<<" "<<tempLL+1<<" "<<tempRL<<"SUM="<<cntplant<<endl;
			if(cntplant>=maxcntplant)
			{
				//maxsum=sum;
				maxcntplant=cntplant;
				maxi=i;
				maxRL=tempRL;
				maxLL=tempLL;
		...
		//out<<"Maximum of everything  "<<maxi<<" "<<maxLL+1<<" "<<maxRL<<"SUM="<<maxcntplant<<endl;
